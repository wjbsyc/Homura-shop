    ...
    <table class="table table-striped">
      ...
      @foreach($cartItems as $item)
        <tr data-id="{{ $item->productSku->id }}">
          ...
          <td>
           <form action="{{ route('cart.remove',['sku'=>$item->productSku->id]) }}" method="post" style="display: inline-block">
          {{ csrf_field() }}
          {{ method_field('DELETE') }}
         <button class="btn btn-xs btn-danger btn-remove" type="submit">移除</button>
           </form>
          </td>
        </tr>
      @endforeach
    ...
    $('.btn-create-order').click(function () {
      ...
      axios.post('{{ route('orders.store') }}', req)
        .then(function (response) {
          swal('订单提交成功', '', 'success')
          .then(() => {
              location.href = "{{url('/orders/')}}"+ "/"+response.data.id;
          });
        }, function (error) {
